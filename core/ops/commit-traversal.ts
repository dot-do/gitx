 ...
 *
 * ```typescript
 * import { walkCommits, CommitWalker } from '@dotdo/gitx/ops'
 *
...
import type { CommitProvider, CommitInfo } from '../types'
...
export interface TraversalCommit {
  /** The commit info with full data */
  commit: CommitInfo
...
/**
 * Extended commit provider with path filtering support
 */
export interface ExtendedCommitProvider extends CommitProvider {
...
export async function* walkCommits(
  provider: ExtendedCommitProvider,
  start: string | string[],
  ...
  let allCommits: TraversalCommit[] = []
  ...
  try {
    ...
    for (const traversalCommit of sortedCommits) {
      ...
      // Merge filter
      if (!includeMerges && commit.parentShas.length > 1) {
        continue
      }
      ...
      // Committer filter (use author as proxy since CommitInfo doesn't have separate committer)
      if (committer && commit.author.name !== committer) {
        continue
      }

      // Date filters
      const commitTimestamp = commit.author.timestamp * 1000
...
export async function topologicalSort(
  ...
  const commitData = new Map<string, CommitInfo>()
  ...
  queue.sort((a, b) => {
    ...
    return commitB.author.timestamp - commitA.author.timestamp
  })
          ...
          if (newDegree === 0) {
            ...
            if (parentCommit) {
              ...
              for (let i = 0; i < queue.length; i++) {
                ...
                if (
                  queueCommit &&
                  parentCommit.author.timestamp <=
                    queueCommit.author.timestamp
                ) {
                  insertIndex = i + 1
                } else {
                  break
...
export async function sortByDate(
  ...
  for (const sha of commits) {
    const commit = await provider.getCommit(sha)
    if (commit) {
      // CommitInfo only has author, use author timestamp
