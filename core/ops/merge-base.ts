 ...
 *
 * ```typescript
 * import { findMergeBase, isAncestor } from '@dotdo/gitx/ops'
 *
...
import type { CommitProvider } from '../types'
...
export async function findForkPoint(
    ...
    for (const entry of reflog) {
      if (refAncestors.has(entry)) {
        ...
        while (current && current !== entry) {
          const commit = await provider.getCommit(current)
          if (!commit || commit.parentShas.length === 0) break
          commitsSinceFork++
          current = commit.parentShas[0]
        }
  ...
  while (current !== forkPoint && !visited.has(current)) {
    ...
    if (commit.parentShas.length === 0) break
    const nextParent = commit.parentShas[0]
    if (!nextParent) break
    current = nextParent
  }
...
export async function checkAncestor(
  ...
  while (queue.length > 0) {
    ...
    for (const parent of commitObj.parentShas) {
      if (!visited.has(parent)) {
