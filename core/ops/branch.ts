 ...
 *
 * ```typescript
 * import { createBranch, checkoutBranch, listBranches } from '@dotdo/gitx/ops'
 *
...
export function isValidBranchName(name: string): boolean {
  ...
  // Cannot contain space, ~, ^, :, \, ?, *, [, control characters
  if (/[\s~^:\\\?\*\[]/.test(name)) {
    return false
  }

  // Cannot contain control characters (ASCII 0-31 and 127)
