...
const lfsInteropCache = new WeakMap<GitRepoDOInstance, LfsInterop>()
 ...
 */
function getLfsInterop(instance: GitRepoDOInstance, baseUrl: string): LfsInterop | null {
  // Check cache first
...
export async function handleLfsBatch(
  ...
  try {
    ...
    if (!body.operation || !['download', 'upload'].includes(body.operation)) {
      return c.json(
        { message: 'Invalid operation. Must be "download" or "upload".' },
        422
      )
    }
    ...
    for (const obj of body.objects) {
      if (!obj.oid || typeof obj.oid !== 'string' || obj.oid.length !== 64) {
        return c.json(
          { message: `Invalid oid: ${obj.oid}. Must be a 64-character SHA256 hash.` },
          422
        )
      }
      if (typeof obj.size !== 'number' || obj.size < 0) {
        return c.json(
          { message: `Invalid size for oid ${obj.oid}. Must be a non-negative number.` },
          422
        )
      }
    }
 ...
 */
export async function handleLfsUpload(
  c: RouteContext,
  instance: GitRepoDOInstance
): Promise<Response> {
  try {
    ...
    if (!oid || !/^[0-9a-f]{64}$/.test(oid)) {
      return c.json(
        { message: 'Invalid OID. Must be a 64-character lowercase hex SHA256 hash.' },
        400
      )
    }
 ...
 * @param instance - GitRepoDO instance
 */
export function setupLfsRoutes(
