 ...
 */
export function matchesProtectionPattern(refName: string, pattern: string): boolean {
  if (pattern === '*') return true
  if (!pattern.includes('*')) return refName === pattern
...
export function checkBranchProtection(
  ...
  for (const rule of rules) {
    ...
    if (!matchesProtectionPattern(update.refName, rule.pattern)) continue
...
export class BranchProtectionManager {
  ...
  upsertRule(input: BranchProtectionInput): number {
    ...
    const rows = this.storage.sql.exec(
      'SELECT id FROM branch_protection WHERE pattern = ?',
      input.pattern
    ...
    if (rows.length > 0) {
      ...
      this.storage.sql.exec(
        `UPDATE branch_protection SET
          required_reviews = ?, prevent_force_push = ?, prevent_deletion = ?, enabled = ?, updated_at = ?
        WHERE id = ?`,
        input.requiredReviews ?? 0,
        input.preventForcePush !== false ? 1 : 0,
        input.preventDeletion !== false ? 1 : 0,
    ...
    this.storage.sql.exec(
      `INSERT INTO branch_protection (pattern, required_reviews, prevent_force_push, prevent_deletion, enabled, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      input.pattern,
      input.requiredReviews ?? 0,
      input.preventForcePush !== false ? 1 : 0,
  ...
  removeRule(pattern: string): boolean {
    const before = this.storage.sql.exec(
      'SELECT COUNT(*) as cnt FROM branch_protection WHERE pattern = ?',
      pattern
  ...
  getEnabledRules(): BranchProtectionRule[] {
    const rows = this.storage.sql.exec(
      'SELECT * FROM branch_protection WHERE enabled = 1 ORDER BY id'
    ).toArray() as Array<{
      id: number
      pattern: string
      required_reviews: number
      prevent_force_push: number
      prevent_deletion: number
      enabled: number
      created_at: number
      updated_at: number
    }>
  ...
  getAllRules(): BranchProtectionRule[] {
    const rows = this.storage.sql.exec(
      'SELECT * FROM branch_protection ORDER BY id'
    ).toArray() as Array<{
      id: number
      pattern: string
      required_reviews: number
