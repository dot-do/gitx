...
export class RefStorage {
  ...
  constructor(backendOrOptions: RefStorageBackend | RefStorageOptions) {
    ...
    if ('storageBackend' in backendOrOptions && !('readRef' in backendOrOptions)) {
      ...
      const backend = (backendOrOptions as RefStorageOptions).storageBackend
      if (backend) this.storageBackend = backend
    } else {
  ...
  async resolveRef(name: string, options?: ResolveRefOptions): Promise<ResolvedRef> {
    ...
    for (let depth = 0; depth < maxDepth; depth++) {
      ...
      if (ref.type === 'direct') {
        ...
        if (!firstRef) throw new RefError('Empty chain in ref resolution', 'CIRCULAR_REF', name)
        return {
          ref: firstRef,
          sha: ref.target,
          chain
  ...
  async updateHeadWithInfo(
    ...
    try {
      ...
      let operation: 'checkout' | 'reset' | 'commit' | 'merge' | 'rebase' | 'other' = 'other'
   ...
   * ```
   */
  async updateRefWithRetry(
    name: string,
    updateFn: (currentSha: string | null) => Promise<string>,
    options?: {
      maxRetries?: number
      retryDelayMs?: number
    ...
    throw lastError ?? new RefError(
      `Max retries (${maxRetries}) exceeded for ref update: ${name}`,
      'CONFLICT',
      name
    )
  }
  ...
  async batchUpdateRefs(
    ...
    try {
      ...
      const currentRefs: Map<string, Ref | null> = new Map()
      for (const update of sortedUpdates) {
