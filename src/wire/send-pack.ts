...
import { parseCloneUrl, type ParsedCloneUrl } from '../ops/clone'
import type { GitBackend } from '../core/backend'
import { generatePackfile, type PackableObject } from '../pack/generation'
...
export interface RemoteRefAdvertisement {
  ...
  refs: Array<{ name: string; sha: string; peeled?: string }>
  /** Server capabilities */
 ...
 */
export function parseReceivePackCapabilities(capsString: string): ReceivePackCapabilities {
  ...
  for (const part of parts) {
    ...
    else if (part === 'ofs-delta') caps.ofsDelta = true
...
function buildClientCapabilities(
  serverCaps: ReceivePackCapabilities,
  options: PushOptions
): string {
  const caps: string[] = []

  // Only request capabilities the server supports
  if (serverCaps.reportStatus) caps.push('report-status')
  if (serverCaps.sideBand64k) caps.push('side-band-64k')
  if (serverCaps.ofsDelta) caps.push('ofs-delta')
  if (options.atomic && serverCaps.atomic) caps.push('atomic')
...
): Promise<RemoteRefAdvertisement> {
 ...
 */
function parseReceivePackAdvertisement(body: string): RemoteRefAdvertisement {
  const { packets } = pktLineStream(body)

  const refs: Array<{ name: string; sha: string; peeled?: string }> = []
 ...
 */
async function collectObjectsToSend(
  backend: GitBackend,
  refUpdates: RefUpdate[],
  ...
  const remoteHas = new Set<string>(remoteRefs.values())
  ...
  const packableObjects: PackableObject[] = []
 ...
 */
function parseTree(data: Uint8Array): Array<{ mode: string; name: string; sha: string }> {
  ...
  while (offset < data.length) {
    ...
    // Read 20-byte SHA
    if (nullIndex + 21 > data.length) break
 ...
 */
function parseReportStatus(data: Uint8Array): {
  unpackOk: boolean
  refResults: RefUpdateResult[]
} {
  let text: string
  ...
  for (const packet of packets) {
    if (packet.type === 'flush' || !packet.data) continue
...
function demultiplexSideBand(data: Uint8Array): string {
  ...
  while (offset < data.length) {
    ...
    if (isNaN(len) || len < 5 || offset + len > data.length) break
...
export async function push(options: PushOptions): Promise<PushResult> {
  ...
  try {
    ...
    const validatedUpdates: RefUpdate[] = []
    for (const update of refUpdates) {
      // Validate SHA format
    ...
    const objectsToSend = await collectObjectsToSend(
      backend,
      validatedUpdates,
      remoteRefs,
      onProgress
    ...
    const requestBody = buildPushRequest(
      validatedUpdates,
      clientCaps,
      packfile,
      remoteAdvert.capabilities.pushOptions ? pushOptions : undefined
    )
...
export async function pushBranch(
  ...
  if (options?.auth) discoverOpts.auth = options.auth
  if (options?.fetch) discoverOpts.fetch = options.fetch
  ...
  if (options?.auth) pushOpts.auth = options.auth
  if (options?.force !== undefined) pushOpts.force = options.force
  if (options?.onProgress) pushOpts.onProgress = options.onProgress
  if (options?.fetch) pushOpts.fetch = options.fetch
