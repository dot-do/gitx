 ...
 * @internal
 */
function createAuthContext(request: SmartHTTPRequest): AuthContext {
...
export function createAuthMiddleware(
  ...
  return {
    async authenticate(request: SmartHTTPRequest, options?: AuthOptions): Promise<AuthenticationResult> {
    ...
    async authenticateWithHeader(
      ...
      if (!authResult.valid) {
        ...
        return {
          ...
          errorResponse: createUnauthorizedResponse(
            realm,
            authResult.reason || 'Invalid credentials'
          ),
        }
      }
      ...
      if (mergedOptions.requiredScopes && mergedOptions.requiredScopes.length > 0) {
        const hasRequiredScopes = mergedOptions.requiredScopes.every(
...
export interface MemoryAuthProviderConfig {
  ...
  users?: Record<string, { password: string; scopes?: string[]; metadata?: Record<string, unknown> }>
  /** Map of token to token configuration */
  tokens?: Record<string, { scopes?: string[]; userId?: string; metadata?: Record<string, unknown> }>
...
export class MemoryAuthProvider implements AuthProvider {
  private readonly config: MemoryAuthProviderConfig

  constructor(config: MemoryAuthProviderConfig = {}) {
    this.config = config
  }

  async validateCredentials(credentials: Credentials, _context: AuthContext): Promise<AuthResult> {
  ...
  private validateBasicCredentials(username: string, password: string): AuthResult {
...
export class CallbackAuthProvider implements AuthProvider {
  ...
  async validateCredentials(credentials: Credentials, context: AuthContext): Promise<AuthResult> {
    if (credentials.type === 'anonymous') {
      return { valid: false, reason: 'Authentication required' }
    }
 ...
 * ```
 */
export function createAuthenticatedRepositoryProvider<T extends { hasPermission(service: GitService): Promise<boolean> }>(
  provider: T,
  ...
  return new Proxy(provider, {
    get(target, prop) {
      if (prop === 'hasPermission') {
        return async (service: GitService): Promise<boolean> => {
