 ...
 */
export function computeChanges(entries: RefLogEntry[]): Map<string, string> {
 ...
 */
export function findCommonAncestor(ours: DeltaBranch, theirs: DeltaBranch): number {
...
export function threeWayMerge(
  ...
  const merged = new Map<string, RefState>(baseState)
 ...
 */
export function canFastForward(ours: DeltaBranch, theirs: DeltaBranch): 'ours' | 'theirs' | false {
  if (ours.getBranchEntries().length === 0) return 'theirs'
  if (theirs.getBranchEntries().length === 0) return 'ours'
  return false
}
