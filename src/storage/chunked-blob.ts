...
/**
 * Internal blob record stored in the backend.
 */
interface BlobRecord {
  ...
  isChunked: boolean
  chunkKeys: string[]
  data?: Uint8Array // For non-chunked blobs, store data directly
}
 ...
 * ```
 */
export function createChunkedBlobStorage(): ChunkedBlobStorage {
  // In-memory storage for testing
  const chunks = new Map<string, Uint8Array>()
  const metadata = new Map<string, BlobRecord>()

  /**
   * Write a blob, automatically chunking if > CHUNK_SIZE.
   */
  async function writeBlob(data: Uint8Array): Promise<ChunkedWriteResult> {
  ...
  async function readRange(
    ...
    if (offset >= record.totalSize) {
      throw new Error(
        `Range out of bounds: offset=${offset} exceeds size=${record.totalSize}`
      )
    }
    if (offset + length > record.totalSize) {
      throw new Error(
        `Range out of bounds: offset=${offset} + length=${length} exceeds size=${record.totalSize}`
      )
    }
   ...
   * Delete a blob and all its chunks.
   */
  async function deleteBlob(sha: string): Promise<boolean> {
    const record = metadata.get(getMetadataKey(sha))
    if (!record) {
      return false
    }
   ...
   * Check if a blob exists.
   */
  async function hasBlob(sha: string): Promise<boolean> {
   ...
   * Get metadata for a blob.
   */
  async function getMetadata(sha: string): Promise<ChunkMetadata | null> {
    const record = metadata.get(getMetadataKey(sha))
    if (!record) {
      return null
    }
   ...
   */
  async function getChunkKeys(sha: string): Promise<string[]> {
    const record = metadata.get(getMetadataKey(sha))
    if (!record) {
      return []
    }
