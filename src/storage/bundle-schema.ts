...
export class BundleSchemaManager {
   ...
   */
  async getAppliedMigrations(): Promise<Array<{ version: number; description: string; applied_at: number }>> {
    try {
      const result = this.storage.sql.exec(
...
export function recordBundle(
  ...
  bundle: Omit<BundleRecord, 'created_at'> & { created_at?: number }
): void {
  const createdAt = bundle.created_at ?? Date.now()

  storage.sql.exec(
    `INSERT OR REPLACE INTO bundles (id, r2_key, entry_count, size, data_offset, created_at, sealed, checksum)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    bundle.id,
    bundle.r2_key,
    bundle.entry_count,
...
export function lookupBundle(
  ...
  const rows = result.toArray() as BundleRecord[]
  return rows[0] ?? null
}
 ...
 */
export function recordBundleObject(
  storage: DurableObjectStorage,
  obj: Omit<BundleObjectRecord, 'created_at'> & { created_at?: number }
): void {
  const createdAt = obj.created_at ?? Date.now()

  storage.sql.exec(
    `INSERT OR REPLACE INTO bundle_objects
 ...
 */
export function lookupBundleObject(
  storage: DurableObjectStorage,
  key: string
): BundleObjectRecord | null {
  // Look up by key, preferring non-deleted entries
  const result = storage.sql.exec(
    `SELECT * FROM bundle_objects WHERE key = ? AND deleted = 0 ORDER BY created_at DESC LIMIT 1`,
    key
  )
  const rows = result.toArray() as BundleObjectRecord[]
  return rows[0] ?? null
}
 ...
 */
export function getBundleObjects(
  storage: DurableObjectStorage,
  bundleId: string
): BundleObjectRecord[] {
  const result = storage.sql.exec(
    'SELECT * FROM bundle_objects WHERE bundle_id = ? AND deleted = 0 ORDER BY offset',
    bundleId
  )
  return result.toArray() as BundleObjectRecord[]
}
 ...
 */
export function markObjectDeleted(
  storage: DurableObjectStorage,
  key: string
): number {
  const result = storage.sql.exec(
    'UPDATE bundle_objects SET deleted = 1 WHERE key = ?',
    key
  )
  const rows = result.toArray() as Array<{ changes?: number }>
...
export function getActiveBundle(
  storage: DurableObjectStorage
): ActiveBundleRecord | null {
  const result = storage.sql.exec(
    'SELECT * FROM active_bundle WHERE id = 1'
  )
  const rows = result.toArray() as ActiveBundleRecord[]
  return rows[0] ?? null
}
...
export function setActiveBundle(
  ...
  storage.sql.exec(
    `INSERT OR REPLACE INTO active_bundle
 ...
 */
export function updateActiveBundle(
  storage: DurableObjectStorage,
 ...
 * @param storage - Durable Object storage
 */
export function clearActiveBundle(storage: DurableObjectStorage): void {
...
export function getBundleStats(
  ...
  const bundleResult = storage.sql.exec(
    `SELECT
  ...
  const bundleStats = bundleResult.toArray() as Array<{
    total_bundles: number
    sealed_bundles: number
    total_size: number
  }>

  const objectResult = storage.sql.exec(
    `SELECT
       COUNT(*) as total_objects,
       SUM(CASE WHEN deleted = 1 THEN 1 ELSE 0 END) as deleted_objects
     FROM bundle_objects`
  )
  const objectStats = objectResult.toArray() as Array<{
...
export function listBundles(
  ...
  return result.toArray() as BundleRecord[]
}
