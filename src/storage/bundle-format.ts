 ...
 */
function oidToBytes(oid: string): Uint8Array {
 ...
 */
function bytesToOid(bytes: Uint8Array): string {
  return Array.from(bytes)
    .map((b) => b.toString(16).padStart(2, '0'))
    .join('')
}

function computeHeaderChecksum(data: Uint8Array): Uint8Array {
...
function computeBundleChecksum(data: Uint8Array): Uint8Array {
...
function verifyHeaderChecksum(data: Uint8Array, expectedChecksum: Uint8Array): boolean {
  ...
  for (let i = 0; i < computed.length; i++) {
    if (computed[i] !== expectedChecksum[i]) return false
  }
  return true
}

function verifyBundleChecksum(bundleData: Uint8Array): boolean {
...
export function parseBundleIndex(data: Uint8Array, entryCount: number): BundleIndexEntry[] {
  ...
  if (data.length < expectedSize) {
    throw new BundleIndexError(
      `Entry count mismatch: expected ${entryCount} entries (${expectedSize} bytes), but index data is only ${data.length} bytes`
    )
  }
...
export function parseBundle(data: Uint8Array, options?: { verify?: boolean }): Bundle {
  ...
  if (data.length < BUNDLE_HEADER_SIZE) {
    throw new BundleCorruptedError(
      `Bundle truncated: expected at least ${BUNDLE_HEADER_SIZE} bytes for header, got ${data.length}`
    )
  }
  ...
  if (data.length < header.totalSize) {
    throw new BundleCorruptedError(
      `Bundle truncated: expected ${header.totalSize} bytes, got ${data.length}`
    )
  }
  ...
  if (requiredIndexSize > availableIndexSize) {
    throw new BundleCorruptedError(
      `Corrupted entry count: ${header.entryCount} entries require ${requiredIndexSize} bytes but only ${availableIndexSize} available`
    )
  }

  // Parse index
  const indexData = data.slice(header.indexOffset)
  let entries: BundleIndexEntry[]
  ...
  if (options?.verify) {
    for (const entry of entries) {
      ...
      if (entry.offset + entry.size > header.indexOffset) {
        throw new BundleCorruptedError(
          `Entry ${entry.oid} extends beyond data section: offset=${entry.offset}, size=${entry.size}, indexOffset=${header.indexOffset}`
        )
      }
    }
    ...
    for (const entry of entries) {
      ...
      if (entry.offset < BUNDLE_HEADER_SIZE || entry.offset >= header.indexOffset) {
        throw new BundleCorruptedError(
          `Entry ${entry.oid} has invalid offset ${entry.offset}`
        )
      }
    }
    ...
    for (const entry of sortedByOffset) {
      if (entry.offset !== expectedOffset) {
        throw new BundleCorruptedError(
          `Data section integrity error: expected entry at offset ${expectedOffset}, but found entry ${entry.oid} at offset ${entry.offset}`
        )
      }
    ...
    if (expectedOffset !== header.indexOffset) {
      throw new BundleCorruptedError(
        `Data section size mismatch: entries end at ${expectedOffset}, but index starts at ${header.indexOffset}`
      )
    }
  }
...
export class BundleWriter {
  ...
  private currentSize: number = BUNDLE_HEADER_SIZE // Start with header size
