...
export function oidToBytes(oid: string): Uint8Array {
...
export function bytesToOid(bytes: Uint8Array): string {
...
function computeChecksum(data: Uint8Array, dataLength: number): Uint8Array {
...
export function computeBundleChecksum(data: Uint8Array): Uint8Array {
  const checksum = new Uint8Array(16)
...
export function verifyBundleChecksum(bundleData: Uint8Array): boolean {
  const expected = bundleData.slice(48, 64)
  const computed = computeBundleChecksum(bundleData)
  if (computed.length !== expected.length) return false
  for (let i = 0; i < computed.length; i++) {
    if ((computed[i] ?? 0) !== (expected[i] ?? 0)) return false
  }
  return true
}
...
export function parseBundleIndex(data: Uint8Array, entryCount: number): BundleIndexEntry[] {
  ...
  if (data.length < expectedSize) {
    throw new BundleIndexError(
      `Index truncated: expected ${expectedSize} bytes for ${entryCount} entries, got ${data.length}`
    )
  }
...
export function createBundle(
  ...
  let dataSize = 0
  for (const obj of sorted) dataSize += obj.data.length
...
export function parseBundle(data: Uint8Array, options?: { verify?: boolean }): Bundle {
  if (data.length < BUNDLE_HEADER_SIZE) {
    throw new BundleCorruptedError(
      `Bundle truncated: expected at least ${BUNDLE_HEADER_SIZE} bytes, got ${data.length}`
    )
  }
  ...
  if (requiredIndexSize > availableIndexSize) {
    throw new BundleCorruptedError(
      `Index overflow: ${header.entryCount} entries need ${requiredIndexSize} bytes, only ${availableIndexSize} available`
    )
  }
  ...
  if (options?.verify) {
    for (const entry of entries) {
      if (entry.offset + entry.size > header.indexOffset) {
        throw new BundleCorruptedError(
          `Entry ${entry.oid} extends beyond data section: offset=${entry.offset}, size=${entry.size}`
        )
      }
...
export function objectTypeToBundleType(type: string): BundleObjectType {
  switch (type) {
...
export function bundleTypeToObjectType(type: BundleObjectType): string {
  switch (type) {
