 ...
 */
export const REPOSITORY_FIELDS: ParquetField[] = [
  // -------------------------------------------------------------------------
  // Identity
  // -------------------------------------------------------------------------
  {
 ...
 * ```
 */
export const REPOSITORIES_SCHEMA: ParquetSchema = defineSchema(REPOSITORY_FIELDS, {
 ...
 */
export function toRepositoryRow(
  repo: {
    fullName: string
    source: RepositorySource
    sourceId?: string
  ...
  const toTimestamp = (d: Date | number | undefined): number | null => {
 ...
 */
export function toNamespace(source: RepositorySource, fullName: string): string {
 ...
 */
export function fromNamespace(namespace: string): { source: RepositorySource; fullName: string } {
  const colonIndex = namespace.indexOf(':')
  if (colonIndex === -1) {
    return { source: 'github', fullName: namespace }
  }
  return {
    source: namespace.slice(0, colonIndex) as RepositorySource,
