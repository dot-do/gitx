 ...
 */
export const REF_FIELDS: ParquetField[] = [
  // Reference identity
  {
    name: 'name',
 ...
 * ```
 */
export const REFS_SCHEMA: ParquetSchema = defineSchema(REF_FIELDS, {
  table_name: 'refs',
  version: '1.0',
  created_by: 'gitx',
})
 ...
 */
function getRefType(name: string): RefRow['ref_type'] {
...
export function toRefRow(
  ref: {
    ...
    behind?: number
    tagMessage?: string
    tagger?: { name: string; email: string; date: Date | number }
  },
  repository: string,
  options: {
    isHead?: boolean
