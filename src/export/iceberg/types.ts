...
export interface PartitionField {
  ...
  field_id: number
  name: string
  transform: PartitionTransform | { type: 'bucket' | 'truncate'; width: number }
}
...
export interface DataFile {
  content: 0 | 1 | 2 // 0=data, 1=position delete, 2=equality delete
  file_path: string
...
export type TableUpdate =
  ...
  | { action: 'set-current-schema'; schema_id: number }
  | { action: 'add-partition-spec'; spec: PartitionSpec }
  | { action: 'set-default-spec'; spec_id: number }
  | { action: 'add-sort-order'; sort_order: SortOrder }
  | { action: 'set-default-sort-order'; sort_order_id: number }
  | { action: 'add-snapshot'; snapshot: IcebergSnapshot }
  | { action: 'set-snapshot-ref'; ref_name: string; type: 'branch' | 'tag'; snapshot_id: number }
  | { action: 'remove-snapshots'; snapshot_ids: number[] }
...
export type TableRequirement =
  ...
  | { type: 'assert-ref-snapshot-id'; ref: string; snapshot_id: number | null }
  | { type: 'assert-last-assigned-field-id'; last_assigned_field_id: number }
  | { type: 'assert-current-schema-id'; current_schema_id: number }
  | { type: 'assert-last-assigned-partition-id'; last_assigned_partition_id: number }
  | { type: 'assert-default-spec-id'; default_spec_id: number }
  | { type: 'assert-default-sort-order-id'; default_sort_order_id: number }
