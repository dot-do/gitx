...
import { TreeEntry, isValidMode, isValidSha } from '../types/objects'
 ...
 * ```
 */
export function validateTreeEntry(entry: TreeEntry): TreeEntryValidationResult {
 ...
 * ```
 */
export function validateTreeEntries(entries: TreeEntry[]): TreeEntriesValidationResult {
  const errors: string[] = []
 ...
 * ```
 */
export function assertValidTreeEntries(entries: TreeEntry[]): void {
  const seenNames = new Set<string>()
 ...
 */
export function isDirectoryEntry(entry: TreeEntry): boolean {
 ...
 */
export function isFileEntry(entry: TreeEntry): boolean {
 ...
 */
export function isSymlinkEntry(entry: TreeEntry): boolean {
 ...
 */
export function isSubmoduleEntry(entry: TreeEntry): boolean {
 ...
 * ```
 */
export function serializeTreeEntry(entry: TreeEntry): Uint8Array {
 ...
 * ```
 */
export function parseTreeEntries(data: Uint8Array): ParseTreeResult {
 ...
 */
export function parseTreeEntriesOrEmpty(data: Uint8Array): TreeEntry[] {
  const result = parseTreeEntries(data)
 ...
 * ```
 */
export function prepareTreeContent(
  entries: TreeEntry[],
...
export interface TreeObjectStore {
  getTreeObject(sha: string): Promise<{ entries: TreeEntry[] } | null>
}
...
export async function walkTree(
  store: TreeObjectStore,
  treeSha: string,
  prefix: string = ''
): Promise<TreeEntryWithPath[]> {
  const tree = await store.getTreeObject(treeSha)
  if (!tree) {
    return []
  }

  const results: TreeEntryWithPath[] = []
 ...
 * ```
 */
export async function getTreeEntryByPath(
  store: TreeObjectStore,
  treeSha: string,
  path: string
 ...
 * ```
 */
export async function collectTreeShas(
  store: TreeObjectStore,
  treeSha: string
): Promise<Set<string>> {
  const result = new Set<string>()
  const queue: string[] = [treeSha]
 ...
 * ```
 */
export async function collectBlobShas(
  store: TreeObjectStore,
  treeSha: string
): Promise<Set<string>> {
  const result = new Set<string>()
