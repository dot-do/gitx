 ...
 * ```
 */
export async function* safeIterate<T>(
  iterable: AsyncIterable<T>,
  options: SafeIteratorOptions = {}
...
export async function forEachAsync<T>(
  iterable: AsyncIterable<T>,
  processor: (item: T, index: number) => boolean | Promise<boolean>,
  options: SafeIteratorOptions = {}
): Promise<number> {
  const { limit = Infinity, onCleanup } = options
  const iterator = iterable[Symbol.asyncIterator]()
 ...
 * ```
 */
export function createDisposableIterator<T>(
  iterable: AsyncIterable<T>
): {
 ...
 * ```
 */
export function withCleanup<Args extends unknown[], T>(
  generatorFn: (...args: Args) => AsyncGenerator<T>,
  name?: string
): (...args: Args) => AsyncGenerator<T> {
  const label = name ?? generatorFn.name ?? 'anonymous'

  return function (...args: Args): AsyncGenerator<T> {
    const generator = generatorFn(...args)

    async function* wrapper(): AsyncGenerator<T> {
      try {
        yield* generator
      } finally {
