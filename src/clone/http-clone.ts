...
/**
 * Delta object needing resolution.
 */
 ...
 */
async function discoverRefs(url: string): Promise<{
  refs: AdvertisedRef[]
  capabilities: string[]
}> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch refs: ${response.status} ${response.statusText}`
    )
  }
 ...
 */
function parseRefAdvertisement(data: Uint8Array): {
  refs: AdvertisedRef[]
  capabilities: string[]
} {
  ...
  for (const packet of packets) {
    ...
    const ref: AdvertisedRef = { sha, name: refName }
...
async function fetchPackfile(
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch packfile: ${response.status} ${response.statusText}`
    )
  }
 ...
 */
function buildUploadPackRequest(
  refs: AdvertisedRef[],
 ...
 */
function extractPackfile(response: Uint8Array): Uint8Array {
 ...
 */
function reassembleSideBandPack(
  response: Uint8Array,
  startOffset: number
): Uint8Array {
  ...
  while (offset < response.length) {
    ...
    const length = parseInt(hexLength, 16)
    if (isNaN(length) || length < 5) break // Need at least header + 1 byte
...
async function unpackAndStore(
  ...
  const objectsByOffset = new Map<number, { type: PackObjectType; data: Uint8Array }>()

  // Track objects by SHA for ref_delta resolution
  const objectsBySha = new Map<string, { type: PackObjectType; data: Uint8Array }>()

  // Pending deltas that need resolution
  const pendingDeltas: DeltaObject[] = []
  ...
  if (pendingDeltas.length > 0) {
    throw new Error(
      `Failed to resolve ${pendingDeltas.length} delta objects - missing base objects`
    )
  }
