...
function layout(title: string, body: string, breadcrumbs?: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
function fileIcon(mode: string): string {
...
async function listAllRefs(instance: GitRepoDOInstance): Promise<Array<{ name: string; sha: string }>> {
  try {
    const storage = instance.getStorage()
    const result = storage.sql.exec('SELECT name, sha FROM refs ORDER BY name')
    return result.toArray() as Array<{ name: string; sha: string }>
  } catch {
    return []
  }
}
...
async function resolveTreePath(
  ...
  treeSha: string,
  pathParts: string[]
): Promise<{ tree: TreeObject; entry?: TreeEntry } | null> {
...
async function walkCommitLog(
  instance: GitRepoDOInstance,
  startSha: string,
  limit: number
): Promise<Array<{ sha: string; commit: CommitObject }>> {
  const store = instance.getObjectStore()
  const commits: Array<{ sha: string; commit: CommitObject }> = []
...
export function setupWebRoutes(
...
async function computeTreeDiff(
  ...
  if (!newTree) return diffs

  const oldEntries = new Map<string, TreeEntry>()
  if (oldTree) {
...
function renderDiff(diffs: FileDiff[]): string {
  let html = ''
