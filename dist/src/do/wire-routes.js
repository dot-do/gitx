 ...
 */
export class DORepositoryProvider {
    storage;
    objectStore;
    schemaManager;
    ...
    async getRefs() {
        ...
        try {
            const result = this.storage.sql.exec("SELECT name, target FROM refs ORDER BY name");
    ...
    async uploadPack(wants, haves, _capabilities) {
        await this.ensureSchema();
    ...
    async receivePack(packData, commands) {
        await this.ensureSchema();
     ...
     */
    async unpackPackfileIntoTransaction(packData, tx) {
        ...
        const resolveExternalBase = async (sha) => {
        ...
        console.log(`[WireProtocol] unpackPackfileIntoTransaction: version=${result.version}, objects=${result.objectCount}, bytes=${packData.length}`);
        // Buffer each unpacked object in the transaction
        for (const obj of result.objects) {
            tx.bufferObject(obj.sha, obj.type, obj.data);
        }
        console.log(`[WireProtocol] unpackPackfileIntoTransaction: buffered ${result.objects.length} objects`);
    }
    /**
     * Creates an adapter satisfying the upload-pack ObjectStore interface,
     * backed by the DO's SQLite object store.
     */
    createUploadPackStore() {
        ...
        return {
            ...
            async getReachableObjects(sha, _depth) {
 ...
 */
async function toSmartHTTPRequest(c, path, repository) {
...
function toWebResponse(smartResponse) {
    return new Response(smartResponse.body, {
 ...
 */
export function setupWireRoutes(router, instance) {
    ...
    router.post('/:namespace/git-upload-pack', async (c) => {
        try {
            ...
            // Use cached DORepositoryProvider from DO instance (avoids per-request recreation)
            const provider = instance.getRepositoryProvider();
            const request = await toSmartHTTPRequest(c, '/git-upload-pack', namespace);
    ...
    router.post('/:namespace/git-receive-pack', async (c) => {
        try {
            ...
            // Use cached DORepositoryProvider from DO instance (avoids per-request recreation)
            const provider = instance.getRepositoryProvider();
            const request = await toSmartHTTPRequest(c, '/git-receive-pack', namespace);
...
//# sourceMappingURL=wire-routes.js.map