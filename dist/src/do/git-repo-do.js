...
export class GitRepoDO extends DO {
     ...
     */
    _cachedSchemaManager;
    _cachedObjectStore;
    _cachedRepositoryProvider;
    _cachedGitBackendAdapter;
        ...
        if (env.ENABLE_RATE_LIMIT) {
            routeOptions.rateLimit = {
                store: env.RATE_LIMIT_DO
                    ? new DORateLimitStore(env.RATE_LIMIT_DO)
    ...
    async initialize(options) {
        ...
        try {
            ...
            throw new GitRepoDOError(`Invalid namespace URL: ${options.ns}`, GitRepoDOErrorCode.INVALID_NAMESPACE, { ns: options.ns });
        }
        this._ns = options.ns;
    ...
    async compact() {
        ...
        if (!this._initialized) {
            ...
            throw new GitRepoDOError('Cannot compact: DO not initialized', GitRepoDOErrorCode.NOT_INITIALIZED, { ns: this._ns });
        }
        ...
        if (totalItems === 0) {
            ...
            throw new GitRepoDOError('Nothing to compact', GitRepoDOErrorCode.NOTHING_TO_COMPACT, { ns: this._ns });
        }
        const result = {
     ...
     */
    _recordCompactionFailure(attemptCount, errorMessage) {
        const now = Date.now();
        this.state.storage.sql.exec(`INSERT OR REPLACE INTO ${COMPACTION_RETRIES_TABLE} (id, attempt_count, last_error, updated_at) VALUES (1, ?, ?, ?)`, attemptCount, errorMessage, now);
    }
    /**
     * Reset the compaction attempt counter (called on success).
     */
    _resetCompactionAttempts() {
        this.state.storage.sql.exec(`DELETE FROM ${COMPACTION_RETRIES_TABLE} WHERE id = 1`);
    }
    /**
     ...
     */
    scheduleCompaction(delayMs = 10_000) {
        if (!this._parquetStore)
            return false;
        ...
        this.state.waitUntil(this._parquetStore.runCompactionIfNeeded().catch(err => this._logger.error('Inline compaction fallback failed', {
