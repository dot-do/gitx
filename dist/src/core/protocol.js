 ...
 */
export function parsePktLine(data) {
    if (data.length < 4) {
        throw new Error('Incomplete pkt-line: not enough bytes for length');
    }
 ...
 */
export function getCapabilityValue(caps, name) {
 ...
 */
export function parseRefAdvertisements(lines) {
    const refs = [];
    let capabilities = [];
    let isFirst = true;
 ...
 */
export function formatRefAdvertisements(refs, capabilities) {
    const lines = [];
    if (refs.length === 0) {
