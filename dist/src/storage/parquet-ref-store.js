...
export class ParquetRefStore {
     ...
     */
    ensureTable() {
        this.sql.sql.exec('CREATE TABLE IF NOT EXISTS refs (name TEXT PRIMARY KEY, target TEXT NOT NULL, type TEXT DEFAULT \'sha\', updated_at INTEGER)');
    }
    /**
     * Get a ref by name from SQLite (authoritative).
     */
    getRef(name) {
        const result = this.sql.sql.exec('SELECT name, target, type FROM refs WHERE name = ?', name);
        const rows = typedQuery(result, validateRow(['name', 'target', 'type']));
        if (rows.length === 0)
            return null;
        const row = rows[0];
        if (!row)
     ...
     */
    setRef(name, target, type = 'sha') {
        const now = Date.now();
        this.sql.sql.exec('INSERT OR REPLACE INTO refs (name, target, type, updated_at) VALUES (?, ?, ?, ?)', name, target, type, now);
        this.dirty = true;
    }
    /**
     ...
     */
    compareAndSwapRef(name, expectedOldTarget, newTarget) {
        ...
        try {
            ...
            const result = this.sql.sql.exec('SELECT target FROM refs WHERE name = ?', name);
            const rows = typedQuery(result, validateRow(['name', 'target', 'type']));
            ...
            // Apply the update
            const now = Date.now();
            this.sql.sql.exec('INSERT OR REPLACE INTO refs (name, target, type, updated_at) VALUES (?, ?, ?, ?)', name, newTarget, 'sha', now);
    ...
    listRefs(prefix) {
        let result;
        if (prefix) {
            result = this.sql.sql.exec('SELECT name, target, type FROM refs WHERE name LIKE ?', `${prefix}%`);
        }
        else {
...
//# sourceMappingURL=parquet-ref-store.js.map