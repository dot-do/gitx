...
import { BundleObjectType, objectTypeToBundleType, createBundle, BUNDLE_HEADER_SIZE, BUNDLE_INDEX_ENTRY_SIZE, } from './bundle/format';
// =============================================================================
// Error Classes
// =============================================================================
/**
 ...
 */
export var MigrationErrorCode;
(function (MigrationErrorCode) {
    /** Object not found in source storage */
    MigrationErrorCode["OBJECT_NOT_FOUND"] = "OBJECT_NOT_FOUND";
    /** Failed to read object from source */
    MigrationErrorCode["READ_FAILED"] = "READ_FAILED";
    /** Failed to write bundle to destination */
    MigrationErrorCode["WRITE_FAILED"] = "WRITE_FAILED";
    /** Checksum verification failed */
    MigrationErrorCode["CHECKSUM_MISMATCH"] = "CHECKSUM_MISMATCH";
    /** Migration was interrupted */
    MigrationErrorCode["INTERRUPTED"] = "INTERRUPTED";
    /** Invalid migration configuration */
    MigrationErrorCode["INVALID_CONFIG"] = "INVALID_CONFIG";
    /** Failed to acquire lock for object */
    MigrationErrorCode["LOCK_FAILED"] = "LOCK_FAILED";
    /** Checkpoint not found for resume */
    MigrationErrorCode["CHECKPOINT_NOT_FOUND"] = "CHECKPOINT_NOT_FOUND";
    /** Generic migration failure */
    MigrationErrorCode["MIGRATION_FAILED"] = "MIGRATION_FAILED";
 ...
 */
function toBundleObjectType(type) {
    try {
 ...
 */
function extractShaFromKey(key, prefix) {
...
export class LooseToBundleMigrator {
    ...
    async migrate(overrides) {
        ...
        try {
            ...
            throw new MigrationError(`Migration failed: ${err instanceof Error ? err.message : String(err)}`, MigrationErrorCode.MIGRATION_FAILED, err instanceof Error ? err : undefined);
        }
    }
    /**
    ...
    async resume(checkpointId) {
        ...
        if (!checkpoint) {
            throw new MigrationError(`Checkpoint not found: ${checkpointId}`, MigrationErrorCode.CHECKPOINT_NOT_FOUND);
        }
     ...
     */
    async migrateObject(obj, dryRun) {
        // Read the loose object
        const r2Object = await this.r2.get(obj.r2Key);
        if (!r2Object) {
            throw new MigrationError(`Object not found: ${obj.sha}`, MigrationErrorCode.OBJECT_NOT_FOUND);
        }
     ...
     */
    async verifyMigration(originalObjects) {
        const errors = [];
     ...
     */
    async cleanupLooseObjects(objects) {
        let cleaned = 0;
 ...
 */
export async function runMigrationCLI(storage, r2, options = {}) {
    const config = {
