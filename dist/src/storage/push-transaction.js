...
export class BufferOverflowError extends Error {
    ...
    objectBytes;
    constructor(currentBytes, maxBytes, objectBytes) {
        super(`Buffer overflow: adding ${objectBytes} bytes would exceed limit ` +
...
export class PushTransaction {
     ...
     */
    constructor(storage, objectStore, optionsOrCleanup) {
     ...
     */
    bufferObject(sha, type, data) {
        if (this._phase !== 'idle' && this._phase !== 'buffering') {
            throw new Error(`Cannot buffer objects in phase '${this._phase}'. Transaction must be in 'idle' or 'buffering' phase.`);
        }
    ...
    async execute(commands) {
        ...
        try {
            ...
            for (const cmd of commands) {
                ...
                const existing = this.storage.sql.exec('SELECT target FROM refs WHERE name = ?', cmd.refName).toArray();
                const currentSha = existing.length > 0 ? existing[0].target : ZERO_SHA;
                // Verify old SHA matches
                if (currentSha !== cmd.oldSha) {
                    validationErrors.set(cmd.refName, isCreate
            ...
            if (validationErrors.size > 0) {
                ...
                if (orphanedShas.length > 0 && this.orphanCleanup) {
                    console.warn(`[PushTransaction] ${orphanedShas.length} potentially orphaned objects after atomic push failure:`, orphanedShas);
            ...
            for (const cmd of commands) {
                ...
                if (isDelete) {
                    ...
                    this.storage.sql.exec('INSERT OR REPLACE INTO refs (name, target, type, updated_at) VALUES (?, ?, ?, ?)', cmd.refName, cmd.newSha.toLowerCase(), 'sha', txTimestamp);
                }
            ...
            if (orphanedShas.length > 0 && this.orphanCleanup) {
                console.warn(`[PushTransaction] ${orphanedShas.length} potentially orphaned objects after transaction error:`, orphanedShas);
                this.orphanCleanup.scheduleOrphanCleanup(orphanedShas);
            }
            return {
                success: false,
        ...
        if (orphanedShas.length > 0) {
            console.warn(`[PushTransaction] ${orphanedShas.length} potentially orphaned objects after ref update failures:`, orphanedShas);
     ...
     */
    identifyOrphanedShas(results, commands) {
        const ZERO_SHA = '0000000000000000000000000000000000000000';
    ...
    rollbackRefLogEntries() {
        ...
        if (removed > 0) {
            console.debug(`[PushTransaction] Rolled back ${removed} RefLog entries from version ${minVersion}`);
        }
...
//# sourceMappingURL=push-transaction.js.map