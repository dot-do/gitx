 ...
 * ```
 */
export function calculateChunkCount(size, chunkSize = CHUNK_SIZE) {
    if (size === 0)
        return 0;
 ...
 * ```
 */
export function getChunkRange(offset, length, chunkSize = CHUNK_SIZE) {
 ...
 * ```
 */
export function getChunkKey(sha, chunkIndex, prefix = CHUNKED_BLOB_PREFIX) {
 ...
 * ```
 */
export function getMetadataKey(sha, prefix = CHUNKED_BLOB_PREFIX) {
    return `${prefix}${sha}`;
}
/**
 ...
 * ```
 */
export function getAllChunkKeys(sha, chunkCount, prefix = CHUNKED_BLOB_PREFIX) {
    const keys = [];
 ...
 * ```
 */
export function reassembleChunks(chunks, totalSize) {
    if (chunks.length === 0) {
    ...
    if (totalSize !== undefined && actualSize !== totalSize) {
        throw new Error(`Chunk reassembly size mismatch: expected ${totalSize}, got ${actualSize}`);
    }
    // Reassemble
 ...
 * ```
 */
export function extractRange(chunks, offset, length, chunkSize = CHUNK_SIZE) {
    if (length === 0) {
        return new Uint8Array(0);
    }
    ...
    if (offset + length > totalSize) {
        throw new Error(`Range out of bounds: offset=${offset} + length=${length} exceeds size=${totalSize}`);
    }
