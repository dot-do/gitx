...
(function (StreamChannel) {
    ...
    StreamChannel[StreamChannel["PACK_DATA"] = 1] = "PACK_DATA";
 ...
 * ```
 */
export function createBlobReadStream(data, options = {}) {
 ...
 * ```
 */
export function createR2ReadStream(r2Object) {
 ...
 * ```
 */
export function createStreamingPackWriter(objectCount, options = {}) {
...
export class StreamingPackWriter {
    ...
    hashState = [];
    finalized = false;
    constructor(objectCount, options = {}) {
    ...
    async writeStreamingObject(_sha, type, size, dataStream) {
        ...
        const deflater = new pako.Deflate({ level: this.compressionLevel });
        try {
            while (true) {
    ...
    async finalize() {
        ...
        if (this.objectsWritten !== this.objectCount) {
            throw new Error(`Expected ${this.objectCount} objects, but wrote ${this.objectsWritten}`);
        }
        this.finalized = true;
 ...
 * ```
 */
export function createStreamingPackReader(options = {}) {
    let buffer = new Uint8Array(0);
    ...
    return new WritableStream({
        async write(chunk) {
            ...
            while (headerParsed && objectsParsed < objectCount && buffer.length > 0) {
                ...
                let resolvedType = isObjectType(type) ? type : null;
 ...
 */
function tryParseObject(buffer, objectIndex) {
    if (buffer.length < 2)
        return null;
    let offset = 0;
 ...
 */
export function isLargeBlob(size) {
 ...
 */
export function createProgressTransform(onProgress, totalSize) {
    let bytesProcessed = 0;
    return new TransformStream({
        transform(chunk, controller) {
            bytesProcessed += chunk.length;
