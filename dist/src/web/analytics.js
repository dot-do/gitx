 ...
 */
async function walkCommits(provider, startSha, maxCommits = 500) {
    const commits = [];
 ...
 */
async function collectTreePaths(store, treeSha, prefix = '') {
 ...
 */
async function gatherAnalytics(store, provider) {
...
function renderDashboard(data) {
    const langRows = data.languages
        .slice(0, 20)
        .map((l) => `<tr><td>${esc(l.language)}</td><td>${l.fileCount}</td><td>${l.percentage}%</td></tr>`)
        .join('\n');
    const contribRows = data.contributors
        .slice(0, 30)
        .map((c) => `<tr><td>${esc(c.name)}</td><td>${esc(c.email)}</td><td>${c.commits}</td><td>${fmtDate(c.lastCommit)}</td></tr>`)
        .join('\n');
...
function renderBarChart(items, color = 'var(--accent)') {
    if (items.length === 0)
...
function renderLangBar(languages) {
 ...
 */
export function setupAnalyticsRoutes(router, getContext) {
    router.get('/analytics', async (c) => {
        try {
            ...
            return c.html(`<html><body style="background:#0d1117;color:#f85149;padding:2rem;font-family:sans-serif"><h1>Analytics Error</h1><pre>${esc(message)}</pre></body></html>`, 500);
        }
    });
