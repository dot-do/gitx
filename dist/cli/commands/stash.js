 ...
 */
function formatStashRef(index) {
    return `stash@{${index}}`;
}
/**
 * Read the stash reflog to get all stash entries
 */
async function readStashReflog(gitDir) {
 ...
 */
async function writeStashReflog(gitDir, entries) {
    const reflogPath = path.join(gitDir, 'logs', 'refs', 'stash');
 ...
 */
async function readMockStashData(gitDir, index) {
 ...
 */
async function writeMockStashData(gitDir, index, data) {
    const stashDataDir = path.join(gitDir, 'mock-stash-data', `stash@{${index}}`);
 ...
 */
async function deleteMockStashData(gitDir, index) {
    const stashDataDir = path.join(gitDir, 'mock-stash-data', `stash@{${index}}`);
    try {
 ...
 */
async function getWorkingDirChanges(cwd, gitDir) {
    const changes = [];
 ...
 */
async function renumberStashes(gitDir, droppedIndex, totalCount) {
 ...
 */
export async function getStashCount(cwd) {
 ...
 */
async function updateStashCount(gitDir, count) {
    const mockCountPath = path.join(gitDir, 'mock-stash-count');
    if (count > 0) {
 ...
 */
export async function stashPush(cwd, options) {
    const gitDir = await findGitDir(cwd);
    const branch = await getCurrentBranch(gitDir);
 ...
 */
export async function stashApply(cwd, options) {
    const gitDir = await findGitDir(cwd);
    const count = await getStashCount(cwd);
    if (count === 0) {
 ...
 */
export async function stashPop(cwd, options) {
    const gitDir = await findGitDir(cwd);
    const count = await getStashCount(cwd);
    if (count === 0) {
        throw new Error('No stash entries');
 ...
 */
export async function stashShow(cwd, ref) {
    const gitDir = await findGitDir(cwd);
    const count = await getStashCount(cwd);
    if (count === 0) {
        throw new Error('No stash entries');
