...
(function (PackObjectType) {
    PackObjectType[PackObjectType["COMMIT"] = 1] = "COMMIT";
    PackObjectType[PackObjectType["TREE"] = 2] = "TREE";
    PackObjectType[PackObjectType["BLOB"] = 3] = "BLOB";
    PackObjectType[PackObjectType["TAG"] = 4] = "TAG";
    // Type 5 is reserved
    PackObjectType[PackObjectType["OFS_DELTA"] = 6] = "OFS_DELTA";
    PackObjectType[PackObjectType["REF_DELTA"] = 7] = "REF_DELTA";
 ...
 */
export function parsePackHeader(data, offset = 0) {
    ...
    const version = (data[offset + 4] << 24) |
        (data[offset + 5] << 16) |
        (data[offset + 6] << 8) |
        data[offset + 7];
    if (version !== 2) {
    ...
    const objectCount = ((data[offset + 8] << 24) |
        (data[offset + 9] << 16) |
        (data[offset + 10] << 8) |
        data[offset + 11]) >>>
        0;
 ...
 */
export function validatePackHeader(data) {
    try {
 ...
 */
export function encodeVariableLengthSize(size) {
    const bytes = [];
    let value = size;
    do {
        let byte = value & 0x7f;
 ...
 */
export function decodeVariableLengthSize(data, offset) {
    let value = 0;
    let shift = 0;
    let bytesRead = 0;
    while (true) {
 ...
 */
export function decodeObjectHeader(data, offset) {
 ...
 */
export function createFanoutTable(entries) {
    const fanout = new Uint32Array(256);
    let count = 0;
    for (let i = 0; i < 256; i++) {
...
export function parsePackIndex(data) {
    ...
    const largeOffsetCount = countLargeOffsets(data, offsetListStart, objectCount);
 ...
 */
export function createPackIndex(entries, packChecksum) {
    // Sort entries by SHA
    const sortedEntries = [...entries].sort((a, b) => a.sha.localeCompare(b.sha));
 ...
 */
export function lookupObjectInIndex(index, sha) {
    // Validate SHA
 ...
 */
export function isLargeOffset(offset) {
    return offset >= LARGE_OFFSET_THRESHOLD;
}
/**
 * Read an 8-byte offset from buffer.
 */
export function readLargeOffset(data, offset) {
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
 ...
 */
export function parseDeltaOffset(data, pos) {
    let byte = data[pos];
 ...
 */
export function encodeDeltaOffset(offset) {
    if (offset <= 0) {
 ...
 */
export class PackObjectIterator {
    data;
    header;
    constructor(data) {
        this.data = data;
...
function countLargeOffsets(data, offsetListStart, count) {
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    let largeCount = 0;
